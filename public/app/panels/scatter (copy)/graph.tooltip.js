  ...
  function GraphTooltip(elem, dashboard, scope, getSeriesFn) {
    ...
    this.getMultiSeriesPlotHoverInfo = function(seriesList, pos) {
      ...
      for (i = 0; i < seriesList.length; i++) {
        ...
        var x_axis = _.unzip(series.data)[0];
        var x_zip_time = _.zip(x_axis, scope.seriesList[0].time);
        var sorted_x_time = _.sortBy(x_zip_time, function(n){
          return n[0];

        });

        var unzipped_sorted = _.unzip(sorted_x_time);
        hoverIndex = this.findHoverIndexFromData(pos.x, unzipped_sorted[0]);
        results.time = unzipped_sorted[1][hoverIndex];
